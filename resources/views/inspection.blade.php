                    ...
                    <div class="row align-items-center">
                        <div class="col-xl-5 col-xxl-12 mr-auto">
                            <div class="d-sm-flex d-block align-items-center">
                                ...
                                <div>
                                    <h4 class="fs-20 text-black">Update Inspection</h4>
                                    <p class="fs-14 mb-0"></p>
                                </div>
                            </div>
                        </div>
                            <form style="float: left; margin-right:20px" action="{{url('https://api.smallsmall.com/api/multiple-inspection')}}" method="post">
                                @method('PUT')
                                @csrf
                                <input type="hidden" name="id" value="{{($singleInspection[0]['id'])}}" class="form-control input-rounded">
                                <input type="hidden" name="inspection_status" value="multiple-bookings">
                                <input type="submit" class="btn btn-primary btn-md mb-2" value="Multiple Inspection">
                            </form>
                            <form action="{{url('https://api.smallsmall.com/api/apartment-not-available')}}" method="post">
                                @method('PUT')
                                @csrf
                                <input type="hidden" name="id" value="{{($singleInspection[0]['id'])}}" class="form-control input-rounded">
                                <input type="hidden" name="inspection_status" value="apartment-not-available">
                                <input type="hidden" name="email" value="{{($singleInspection[0]['email'])}}">
                                <input type="hidden" name="firstName" value="{{($singleInspection[0]['firstName'])}}">
                                <input type="hidden" name="propertyID" value="{{($singleInspection[0]['propertyID'])}}">
                                <input type="hidden" name="propertyTitle" value="{{($singleInspection[0]['propertyTitle'])}}">
                                <input type="submit" class="btn btn-primary btn-md mb-2" value="Apartment Not Available">
                            </form>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="basic-form">
                        <form method="post" action="https://api.smallsmall.com/api/update-inspection-api/">
                            @method('PUT')
                            @csrf
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control input-rounded" value="{{($singleInspection[0]['inspectionID'])}}" >
                            </div>
                            <div class="form-group">
                                <label>Property ID</label>
                                <input type="text" value="{{($singleInspection[0]['propertyID'])}}" class="form-control input-rounded" >
                            </div>
                            <div class="form-group">
                                <label>Property</label>
                                <input type="text" name="propertyTitle" value="{{($singleInspection[0]['propertyTitle'])}}" class="form-control input-rounded" readonly >
                            </div>
                            <div class="form-group">
                                <label>Inspection Date</label>
                                <input type="text" value="{{($singleInspection[0]['inspectionDate'])}}" class="form-control input-rounded" >
                            </div>
                            <div class="form-group">
                                <label>Updated Inspection Date</label>
                                @if($singleInspection[0]['updated_inspection_date'] == '0000-00-00 00:00:00' || $singleInspection[0]['updated_inspection_date'] == '' || $singleInspection[0]['updated_inspection_date'] == 'NULL')
                                <input type="datetime-local" name="updated_inspection_date" class="form-control input-rounded">
                                @else
                                <input name="updated_inspection_date" value="{{($singleInspection[0]['updated_inspection_date'])}}" class="form-control input-rounded">
                                @endif   
                                
                                <div class="form-group">
                                    <label>Assign TSR</label>
                                    <select class="form-control default-select" id="sel1" name="assigned_tsr">
                                        {{-- {{dd($allStaffTSR)}} --}}
                                        @foreach($allStaffTSR as $allStaffTSR_single)
                                            <option value="{{$allStaffTSR_single['adminID']}}">{{$allStaffTSR_single['firstName']}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                {{-- <input type="datetime-local" name="updated_inspection_date" value="{{($singleInspection[0]['updated_inspection_date'])}}" class="form-control input-rounded"> --}}
                                <input type="hidden" name="id" value="{{($singleInspection[0]['id'])}}" class="form-control input-rounded">
                                <input type="hidden" name="userID" value="{{($singleInspection[0]['userID'])}}" class="form-control input-rounded">
                            </div>
